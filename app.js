    ...
    <div class="product-card" data-category="${p.category}" data-name="${p.name}" data-article="${p.article}">
      ...
      <div class="product-info">
        ...
        <div class="product-footer">
          ...
          <button class="btn-order" onclick="addToCart(this, '${p.name.replaceAll("'", String.raw`\'`)}', ${p.id})">
...
async function handleLogin(event) {
  ...
  try {
    ...
    if (data.result === "SUCCESS") {
      ...
      globalThis.currentUserId = data.id;
  ...
  try {
    ...
    if (result === "OK") {
      ...
      if (loginData.result === "SUCCESS") {
        ...
        localStorage.setItem("login", login);
        globalThis.currentUserId = loginData.id;
        document.getElementById('header-username').textContent = login;
        document.getElementById('auth-screen').classList.remove('active');
        document.getElementById('app-screen').classList.add('active');
        loadProducts();
...
async function checkoutCart() {
  if (CART.length === 0 || !globalThis.currentUserId) return;

  await fetch("http://localhost:3000/create-order", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      user_id: globalThis.currentUserId,
      items: CART
    })
  });
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (savedId) {
    globalThis.currentUserId; = Number(savedId);
...
async function loadOrders() {
  if (!globalThis.currentUserId) return;

  try {
    const res = await fetch(`http://localhost:3000/orders/${globalThis.currentUserId}`);
...
function renderOrders(orders) {
  ...
  // Если есть корзина то показываем ее
  if (CART.length > 0) {
    let total = 0;
  ...
  // Если корзины нет то показываем заказы
...
function updateOrderStats(orders) {
  ...
  const transit = orders.filter(o => o.status === 'В пути' || o.status === 'Создан').length;
  const ready   = orders.filter(o => o.status === 'Готов к выдаче').length;
...
async function loadProfile() {
  if (!globalThis.currentUserId) return;
  try {
    const res  = await fetch(`http://localhost:3000/profile/${globalThis.currentUserId}`);
    if (!res.ok) return;
    const data = await res.json();
    renderProfile(data);
  } catch (err) {
...
// ========== Выбор товара из результата строки поиска ==========
